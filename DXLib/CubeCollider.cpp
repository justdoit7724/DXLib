...
bool CubeCollider::IsHit(Geometrics::Ray ray, XMFLOAT3* hitPt)
{
	...
	if (Dot(ray.d, up) < 0)
	{
		Geometrics::Plane upP(center+ up* extent.y, up, Neg(forward), XMFLOAT2(extent.x, extent.z));
	...
	{
		Geometrics::Plane leftP(center- right* extent.x, Neg(right), up, XMFLOAT2(extent.z, extent.y));
	...
	{
		Geometrics::Plane downP(center- up* extent.y, Neg(up), forward, XMFLOAT2(extent.x, extent.z));
	...
	{
		Geometrics::Plane backP(center- forward* extent.z, Neg(forward), up, XMFLOAT2(extent.x, extent.y));
